...
function renderSetup(session){
  const realUsers=store.users(true).filter(u=>u.id!=='usuario-demo'&&!/usuario demo/i.test(u.name)).length,cat=store.catalogs(),syncReady=store.settings.sync?.provider==='supabase'&&store.settings.sync?.supabaseUrl&&store.settings.sync?.anonKey,identityReady=store.settings.sync?.lockUserToEmail===false||Boolean(sync.boundUserId),graphReady=graph.isConfigured()&&graph.isConnected();
  const steps=[
...
function renderForms(){
  ...
  return `<section class="card"><div class="card-header"><div><h2>Diseñador de formularios</h2><span class="small muted">Muestra, oculta, renombra y reorganiza campos sin modificar el código. Los campos esenciales permanecen activos.</span></div>${badge('CONFIGURABLE')}</div><div class="card-body">
    ...
    <div class="alert" style="margin-top:14px"><strong>Consejo para el prototipo:</strong> deje visibles solo los datos que el ingeniero conoce en el momento. Los campos ocultos conservan sus valores automáticos y pueden reactivarse desde aquí.</div>
  </div></section>`;
}

function renderIntegrations(){
  const m=store.settings.integrations?.microsoft||{},t=store.settings.integrations?.telegram||{},gc=graph.config(),connected=graph.isConnected();
  return `<section class="stack">
    <article class="card"><div class="card-header"><div><h2>Microsoft Graph · configuración rápida</h2><span class="small muted">Conecta OneDrive y Outlook mediante OAuth con PKCE. No se utiliza Client Secret en GitHub Pages ni en la futura APK.</span></div>${badge(connected?'CONECTADO':gc.clientId?'LISTO PARA CONECTAR':'PENDIENTE')}</div><div class="card-body">
      <form id="microsoftIntegrationForm"><div class="form-grid">
        ...
        <div class="field span-6"><label>Permisos delegados</label><textarea name="scopes" rows="3">${escapeHtml((m.scopes||gc.scopes).join('\n'))}</textarea></div>
...
export async function renderSettings(app){
  ...
  $('#syncConfigForm',app)?.addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget,d=Object.fromEntries(new FormData(f));try{await store.saveSettings({sync:{provider:d.provider,supabaseUrl:d.supabaseUrl.trim(),anonKey:d.anonKey.trim(),pollSeconds:Math.max(10,Number(d.pollSeconds||20)),allowSelfSignUp:f.elements.allowSelfSignUp.checked,lockUserToEmail:f.elements.lockUserToEmail.checked}});const session=await sync.session().catch(()=>null);if(session)await sync.bindSessionUser(session);sync.startPolling();toast('Configuración de conexión guardada.','success');renderSettings(app)}catch(err){toast(err.message,'error')}});
